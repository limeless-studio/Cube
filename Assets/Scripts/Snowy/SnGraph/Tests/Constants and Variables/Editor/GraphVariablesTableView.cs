...
namespace Snowy.SnGraph.Tests
{
    ...
    public class GraphVariablesTable : TreeView
    {
        ...
        private void DefaultValueGUI(Rect cellRect, GraphVariableRow row, ref RowGUIArgs args)
        {
            ...
            // allow scene objects to be assigned
            if (row.Variable.Type == typeof(UnityEngine.Object))
            {
                row.Variable.objectValue = EditorGUI.ObjectField(cellRect, serializedValue.objectReferenceValue, row.Variable.Type, true);
            }
            else 
            {
